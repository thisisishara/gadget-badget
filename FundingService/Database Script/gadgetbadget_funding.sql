-- phpMyAdmin SQL Dump
-- version 4.7.9
-- https://www.phpmyadmin.net/
--
-- Host: 127.0.0.1:3306
-- Generation Time: Apr 19, 2021 at 04:47 AM
-- Server version: 5.7.21
-- PHP Version: 5.6.35

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
SET AUTOCOMMIT = 0;
START TRANSACTION;
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;

--
-- Database: `gadgetbadget_funding`
--

DELIMITER $$
--
-- Procedures
--
DROP PROCEDURE IF EXISTS `sp_funding_summary_funderid`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_funding_summary_funderid` (IN `fun_id` VARCHAR(10), OUT `no_fundings` INTEGER, OUT `latest_funding` DATETIME, OUT `total_fundings` DECIMAL(20,2), OUT `total_res` INTEGER, OUT `retrieved_date` DATETIME)  BEGIN
    DECLARE readVal INTEGER DEFAULT 0;
  
    SELECT COUNT(`fund_id`) INTO no_fundings FROM `fund` WHERE `funder_id`=fun_id;
    SELECT MAX(`date_funded`) INTO latest_funding FROM `fund` WHERE `funder_id`=fun_id;
    SELECT SUM(`funded_amount`) INTO total_fundings FROM `fund` WHERE `funder_id`=fun_id;
    SELECT COUNT(DISTINCT `research_id`) INTO total_res FROM `fund` WHERE `funder_id`=fun_id;
    SET retrieved_date = CURRENT_TIMESTAMP();
    
END$$

DELIMITER ;

-- --------------------------------------------------------

--
-- Table structure for table `fund`
--

DROP TABLE IF EXISTS `fund`;
CREATE TABLE IF NOT EXISTS `fund` (
  `fund_id` varchar(10) NOT NULL,
  `funder_id` varchar(10) NOT NULL,
  `research_id` varchar(10) NOT NULL,
  `funded_amount` decimal(12,2) NOT NULL,
  `date_funded` datetime DEFAULT CURRENT_TIMESTAMP,
  `service_charge_rate` decimal(12,2) DEFAULT NULL,
  `creditcard_no` char(20) NOT NULL,
  PRIMARY KEY (`fund_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `fund`
--

INSERT INTO `fund` (`fund_id`, `funder_id`, `research_id`, `funded_amount`, `date_funded`, `service_charge_rate`, `creditcard_no`) VALUES
('FD21000002', 'FN21000031', 'RS21000041', '50000.00', '2021-04-19 08:52:41', '2.00', '0000236665452'),
('FD21000003', 'FN21000031', 'RS21000041', '25000.00', '2021-04-19 08:53:12', '2.00', '0000236665452'),
('FD21000004', 'FN21000031', 'RP21000002', '1000.00', '2021-04-19 08:56:24', '2.00', '5555426635212536');

--
-- Triggers `fund`
--
DROP TRIGGER IF EXISTS `tg_fund_insert`;
DELIMITER $$
CREATE TRIGGER `tg_fund_insert` BEFORE INSERT ON `fund` FOR EACH ROW BEGIN
	DECLARE service_charge DECIMAL(12,2);
    SELECT service_charge_rate INTO service_charge FROM service_charges WHERE service_code='FND';
    
	SET NEW.date_funded = CURRENT_TIMESTAMP();
    SET NEW.service_charge_rate = service_charge;
  	INSERT INTO fund_seq (prefix) VALUES ("FD");
    SET NEW.fund_id = CONCAT(CONCAT("FD",RIGHT(CAST(YEAR(CURDATE()) AS CHAR),2)), LPAD(LAST_INSERT_ID(), 6, '0'));
END
$$
DELIMITER ;

-- --------------------------------------------------------

--
-- Table structure for table `fund_seq`
--

DROP TABLE IF EXISTS `fund_seq`;
CREATE TABLE IF NOT EXISTS `fund_seq` (
  `id` int(10) NOT NULL AUTO_INCREMENT,
  `prefix` char(2) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `fund_seq`
--

INSERT INTO `fund_seq` (`id`, `prefix`) VALUES
(1, 'FD'),
(2, 'FD'),
(3, 'FD'),
(4, 'FD');

-- --------------------------------------------------------

--
-- Table structure for table `jwt_config`
--

DROP TABLE IF EXISTS `jwt_config`;
CREATE TABLE IF NOT EXISTS `jwt_config` (
  `jwt_kid` varchar(100) NOT NULL,
  `jwt_public` varbinary(2048) NOT NULL,
  `jwt_private` varbinary(2048) NOT NULL,
  `jwt_algo` varchar(100) NOT NULL DEFAULT 'RSA_USING_SHA256',
  `jwt_lifetime` int(2) NOT NULL DEFAULT '20',
  `jwt_issuer` varchar(100) NOT NULL DEFAULT 'gadgetbadget.user.security.JWTHandler',
  `jwt_audience` varchar(100) NOT NULL DEFAULT 'gadgetbadget.webservices.auth',
  `jwt_date_last_updated` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`jwt_kid`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `jwt_config`
--

INSERT INTO `jwt_config` (`jwt_kid`, `jwt_public`, `jwt_private`, `jwt_algo`, `jwt_lifetime`, `jwt_issuer`, `jwt_audience`, `jwt_date_last_updated`) VALUES
('JWK1', , , 'RSA_USING_SHA256', 20, 'gadgetbadget.user.security.JWTHandler', 'gadgetbadget.webservices.auth', '2021-04-13 10:44:56'),
('JWK2', , , 'RSA_USING_SHA256', 0, 'gadgetbadget.user.security.JWTHandler', 'gadgetbadget.webservices.serviceauth', '2021-04-14 23:16:59');

--
-- Triggers `jwt_config`
--
DROP TRIGGER IF EXISTS `tg_jwtconfig_update`;
DELIMITER $$
CREATE TRIGGER `tg_jwtconfig_update` BEFORE UPDATE ON `jwt_config` FOR EACH ROW BEGIN
    SET NEW.jwt_date_last_updated = CURRENT_TIMESTAMP();
END
$$
DELIMITER ;

-- --------------------------------------------------------

--
-- Table structure for table `service_charges`
--

DROP TABLE IF EXISTS `service_charges`;
CREATE TABLE IF NOT EXISTS `service_charges` (
  `service_code` char(3) NOT NULL,
  `service_description` varchar(400) DEFAULT NULL,
  `service_charge_rate` decimal(12,2) NOT NULL,
  `date_last_updated` datetime DEFAULT CURRENT_TIMESTAMP,
  `last_modified_by` varchar(10) NOT NULL,
  PRIMARY KEY (`service_code`),
  UNIQUE KEY `service_code` (`service_code`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `service_charges`
--

INSERT INTO `service_charges` (`service_code`, `service_description`, `service_charge_rate`, `date_last_updated`, `last_modified_by`) VALUES
('FND', 'Fund Transfer Service Charge', '2.00', '2021-04-06 06:05:51', 'AD21000002');

--
-- Triggers `service_charges`
--
DROP TRIGGER IF EXISTS `tg_service_charges_update`;
DELIMITER $$
CREATE TRIGGER `tg_service_charges_update` BEFORE UPDATE ON `service_charges` FOR EACH ROW BEGIN
    SET NEW.date_last_updated = CURRENT_TIMESTAMP();
END
$$
DELIMITER ;
COMMIT;

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
